...
- UI文言へ装飾目的の絵文字を追加しない。基本操作・状態は `public/icons/core/` の顔なしピクトグラム、料理分野の補助アイコンは `public/icons/ui/`、食材は `public/ingredients/`、料理は `public/dishes/` を使う。
- 食材・料理のイラストには顔を付けてもよいが、操作アイコンには顔を付けない。
- シェフぐまの顔は **目が2つ、鼻が1つ、口はない**。新規素材でも必ずこの造形を維持する。
- 小〜中サイズの既存UI演出ではピクセルアート、大きなヒーロー・オンボーディング・空状態では滑らかなアートを用途に応じて使い分ける。16〜32pxの操作アイコン内にマスコットを入れない。
