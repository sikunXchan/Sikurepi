...
const W = 1920, H = 1280;
const ink = '#40302c', teal = '#17948d', muted = '#806b62';
...
const base = (tone = 'mint') => `
  ...
  <rect width="1920" height="1280" fill="url(#wash)"/>
...
// Tray collection: a large bear tray balances four classic colorways.
{
  ...
  const layers = [];
  await place(layers, 'serving/recipe-tray-bear-v1.png', 65, 462, 750, 508);
  await place(layers, 'dishes/icons/roast_chicken.png', 225, 592, 229, 222);
  await place(layers, 'dishes/icons/salad.png', 487, 599, 155, 161);
  await place(layers, 'dishes/icons/bread_loaf.png', 488, 768, 130, 99);
  let foreground = txt(442, 1024, 'BEAR', 27, muted, 'middle', 'Arial', 5);
  const themes = [
  ...
  for (let i = 0; i < themes.length; i++) {
    ...
    const x = 855 + (i % 2) * 500, y = 407 + Math.floor(i / 2) * 442;
...
// Badges use the exact order and English rank names from the application.
{
  ...
  for (let i = 0; i < ranks.length; i++) {
    ...
    const cx = 238 + (i % 5) * 361, y = 359 + Math.floor(i / 5) * 433;
    if (i === 9) body += `<rect x="${cx - 156}" y="${y - 47}" width="312" height="385" rx="46" fill="#fff3cf"/>`;
    body += txt(cx, y - 12, `LEVEL ${String(i + 1).padStart(2, '0')}`, 19, i === 9 ? '#ad781c' : teal, 'middle', 'Arial', 3);
    await place(layers, `ranks/${asset}.png`, cx - 121, y + 5, 242, 274);
    foreground += txt(cx, y + 316, name, 28, ink, 'middle');
  }
